...
#pragma warning (disable : 3571) // pow(f,e) negative f warning
...
float3 UvwToViewPosition(float3 uvw)
{
	...
	viewPosition.z = pow(viewPosition.z, _depth_ratio);
...
float3 ViewPositionToUvw(float3 viewPosition)
{
	...
	//uvw.z /= _MaxDistanceVolumetric;
	uvw.z *= _InverseMaxDistanceVolumetric;
	uvw.xy /= lerp(fogNearSize, fogFarSize, uvw.z);
	//uvw.z = pow(abs(uvw.z), rcp(_depth_ratio));
	uvw.z = pow(uvw.z, _inverse_depth_ratio);
...
{
    float3 viewVector = mul(os_CameraInvProjection, float4((uv * 2) - 1, 0.0, 1)).xyz;
...
float3 DirectionAndDistanceToWorldPosition(float3 rayDirection, float depth01)
{
    return GetCameraPosition() + rayDirection * pow(depth01, _depth_ratio) * _MaxDistanceVolumetric;
}
