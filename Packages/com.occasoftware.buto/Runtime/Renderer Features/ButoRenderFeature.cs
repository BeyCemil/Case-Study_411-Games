...
namespace OccaSoftware.Buto.Runtime
{
    public class ButoRenderFeature : ScriptableRendererFeature
    {
        class RenderFogPass : ScriptableRenderPass
        {
            ...
            Vector3Int fog_volume_size_cached = new Vector3Int(192, 108, 128);
            Vector3Int volumeSize = Vector3Int.zero;
            Vector3 cellSize = Vector3.zero;

            internal void SetupMaterials(Camera c)
            {
                UnityEngine.Profiling.Profiler.BeginSample("Buto_Setup Materials");
                ...
                UnityEngine.Profiling.Profiler.BeginSample("Buto_Setup Materials_2");
                if (volumeSize != fog_volume_size_cached)
                {
                    recreateTextures = true;
                    fog_volume_size_cached = volumeSize;
                }

                mediaCompute.GetKernelThreadGroupSizes(0, out uint threadGroupSizeX, out uint threadGroupSizeY, out uint threadGroupSizeZ);

                groupsX = GetGroupCount(fog_volume_size_cached.x, threadGroupSizeX);
                groupsY = GetGroupCount(fog_volume_size_cached.y, threadGroupSizeY);
                groupsZ = GetGroupCount(fog_volume_size_cached.z, threadGroupSizeZ);
                UnityEngine.Profiling.Profiler.EndSample();
                UnityEngine.Profiling.Profiler.BeginSample("Buto_Setup Materials_3");
                if (mediaDataRt == null || !mediaDataRt.IsCreated() || recreateTextures)
                {
                    mediaDataRt = new RenderTexture(1, 1, 0)
                    {
                        dimension = TextureDimension.Tex3D,
                        width = fog_volume_size_cached.x,
                        height = fog_volume_size_cached.y,
                        volumeDepth = fog_volume_size_cached.z,
                ...
                if (integratorDataRt == null || !integratorDataRt.IsCreated() || recreateTextures)
                {
                    integratorDataRt = new RenderTexture(mediaDataRt);
                    integratorDataRt.Create();
                }

                if (lightingDataRt == null || !lightingDataRt.IsCreated() || recreateTextures)
                {
                    lightingDataRt = new RenderTexture(mediaDataRt);
                    lightingDataRt.Create();
                }

                if (kvps.TryGetValue(c, out RTData rt))
                {
                    if (rt.lightingDataRtPrev == null || !rt.lightingDataRtPrev.IsCreated() || recreateTextures)
                    {
                        rt.lightingDataRtPrev = new RenderTexture(mediaDataRt);
                        rt.lightingDataRtPrev.Create();
                    }

                    if (rt.mediaDataRtPrev == null || !rt.mediaDataRtPrev.IsCreated() || recreateTextures)
                    {
                        rt.mediaDataRtPrev = new RenderTexture(mediaDataRt);
            ...
            {
                volumeSize = volumetricFog.VolumeSize;
                cellSize = volumetricFog.GetCellSize(volumeSize);
            }
            ...
            RenderTexture mediaDataRt = null;
            RenderTexture lightingDataRt = null;
            RenderTexture integratorDataRt = null;

            ComputeShader mediaCompute = null;
            ComputeShader lightingCompute = null;
            ComputeShader integratorCompute = null;
            int groupsX = 0;
            int groupsY = 0;
            int groupsZ = 0;
            ...
            {
                isolatedBlitTarget = RTHandles.Alloc(Shader.PropertyToID(butoTextureId), name: butoTextureId);
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref isolatedBlitTarget, descriptor, FilterMode.Point, TextureWrapMode.Clamp, name: butoTextureId);
                RenderingUtils.ReAllocateIfNeeded(ref mergeTarget, descriptor, FilterMode.Point, TextureWrapMode.Clamp, name: mergeTargetId);
            }
            ...
            private void SetupCameraProperties(CommandBuffer cmd, Camera camera)
            {
                ...
                Vector2 frustumNearSize = new Vector2(Vector3.Distance(near[1], near[0]), Vector3.Distance(near[2], near[0])) * 0.5f;
                Vector2 frustumFarSize = new Vector2(Vector3.Distance(far[1], far[0]), Vector3.Distance(far[2], far[0])) * 0.5f;

                // Setup fog sizes
                float fog_distance = volumetricFog.maxDistanceVolumetric.value;
                Vector2 fogFarSize = Vector2.Lerp(frustumNearSize, frustumFarSize, (fog_distance - nearClip) / (farClip - nearClip));
                Vector2 fogNearSize = Vector2.zero;
                ...
                toPreviousView = kvps[camera].previousInverseTransform * camera.transform.localToWorldMatrix;

                // Set properties
                cmd.SetGlobalVector(Params.fog_volume_size.Id, new Vector4(volumeSize.x, volumeSize.y, volumeSize.z, 0));
                cmd.SetGlobalVector(Params.fog_cell_size.Id, cellSize);
                ...
                cmd.SetGlobalMatrix(Params.os_CameraInvProjection.Id, camera.nonJitteredProjectionMatrix.inverse);
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                RenderTargetIdentifier mrt = new RenderTargetIdentifier(mediaDataRt);
                RenderTargetIdentifier mrtp = new RenderTargetIdentifier(kvps[camera].mediaDataRtPrev);

                cmd.SetGlobalTexture(Params.MediaData.Id, mrt);
                cmd.SetGlobalTexture(Params.MediaDataPrevious.Id, mrtp);
                cmd.DispatchCompute(mediaCompute, 0, groupsX, groupsY, groupsZ);
                cmd.CopyTexture(mrt, mrtp);

                // Lighting write
                RenderTargetIdentifier lrt = new RenderTargetIdentifier(lightingDataRt);
                RenderTargetIdentifier lrtp = new RenderTargetIdentifier(kvps[camera].lightingDataRtPrev);
                cmd.SetGlobalTexture(Params.LightingDataTex.Id, lrt);
                cmd.SetGlobalTexture(Params.LightingDataTexPrevious.Id, lrtp);
                cmd.DispatchCompute(lightingCompute, 0, groupsX, groupsY, groupsZ);
                cmd.CopyTexture(lrt, lrtp);

                // Integrator write
                cmd.SetGlobalTexture(Params.IntegratorData.Id, new RenderTargetIdentifier(integratorDataRt));
            ...
            private void SetupData(CommandBuffer cmd, Camera camera)
            {
                ...
                SetupMaterialData(cmd);
                SetupAdditionalLightData(cmd, camera);
                SetupVolumeData(cmd, camera);
            }

            private void SetupMaterialData(CommandBuffer cmd)
            {
                ...
                cmd.SetGlobalFloat("_InverseMaxDistanceVolumetric", 1.0f / volumetricFog.maxDistanceVolumetric.value);
                cmd.SetGlobalFloat("_inverse_depth_ratio", 1.0f / volumetricFog.depthRatio.value);
                ...
                cmd.SetGlobalFloat("buto_TemporalIntegration_Lighting", GetIntegrationAmount(volumetricFog.temporalAALighting.value));
                cmd.SetGlobalFloat("buto_TemporalIntegration_Media", GetIntegrationAmount(volumetricFog.temporalAAMedia.value));
                ...
                cmd.SetGlobalTexture(
                    Params.ColorRamp.Id,
                    volumetricFog.colorRamp.value == null ? Texture2D.whiteTexture : volumetricFog.colorRamp.value
                );
                ...
                cmd.SetGlobalFloat(Params._InverseNoiseScale.Id, 1.0f / volumetricFog.noiseTiling.value);
                cmd.SetGlobalFloat(Params._Inverse_AttenuationBoundarySize.Id, 1.0f / volumetricFog.attenuationBoundarySize.value);
            ...
            private float GetIntegrationAmount(float value)
            {
        ...
        private static class Params
        {
            ...
            public static Param _Inverse_AttenuationBoundarySize = new Param("_Inverse_AttenuationBoundarySize");
        }
    }
}
